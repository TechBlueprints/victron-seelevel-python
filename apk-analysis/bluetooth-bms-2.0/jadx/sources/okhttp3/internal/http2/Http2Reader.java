    ...
    public final boolean nextFrame(boolean requireSettings, Handler handler) throws IOException {
        ...
        try {
            ...
            if (requireSettings && iAnd != 4) {
                throw new IOException("Expected a SETTINGS frame but was " + iAnd);
            }
            int iAnd2 = Util.and(this.source.readByte(), 255);
            int i = this.source.readInt() & Integer.MAX_VALUE;
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(Http2.INSTANCE.frameLog(true, i, medium, iAnd, iAnd2));
            }
            switch (iAnd) {
                case 0:
