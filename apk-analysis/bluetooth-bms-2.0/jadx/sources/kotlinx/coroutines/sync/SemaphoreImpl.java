...
final class SemaphoreImpl extends SegmentQueue<SemaphoreSegment> implements Semaphore {
        ...
        while (true) {
            ...
            if (segmentAndMoveHead != null) {
                ...
                if (andSet != SemaphoreKt.CANCELLED) {
                    ...
                    ((CancellableContinuation) andSet).resumeWith(Result.m13constructorimpl(unit));
                    return;
                }
            }
        }
    ...
    final /* synthetic */ Object addToQueueAndSuspend(Continuation<? super Unit> continuation) {
        ...
        SemaphoreSegment semaphoreSegmentAccess$getTail$p = access$getTail$p(this);
        long andIncrement = enqIdx$FU.getAndIncrement(this);
        SemaphoreSegment semaphoreSegmentAccess$getSegment = access$getSegment(this, semaphoreSegmentAccess$getTail$p, andIncrement / SemaphoreKt.SEGMENT_SIZE);
        int i = (int) (andIncrement % SemaphoreKt.SEGMENT_SIZE);
        if (semaphoreSegmentAccess$getSegment == null || semaphoreSegmentAccess$getSegment.acquirers.get(i) == SemaphoreKt.RESUMED || !semaphoreSegmentAccess$getSegment.acquirers.compareAndSet(i, null, cancellableContinuationImpl2)) {
            Unit unit = Unit.INSTANCE;
            Result.Companion companion = Result.INSTANCE;
            cancellableContinuationImpl2.resumeWith(Result.m13constructorimpl(unit));
        } else {
            cancellableContinuationImpl2.invokeOnCancellation(new CancelSemaphoreAcquisitionHandler(this, semaphoreSegmentAccess$getSegment, i));
        }
