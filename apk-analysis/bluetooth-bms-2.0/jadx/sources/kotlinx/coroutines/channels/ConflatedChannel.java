...
public class ConflatedChannel<E> extends AbstractChannel<E> {
    ...
    private final ReceiveOrClosed<?> sendConflated(E element) {
        ...
        AbstractSendChannel.SendBuffered<? extends E> sendBuffered = new AbstractSendChannel.SendBuffered<>(element);
    ...
    private final void conflatePreviousSendBuffered(AbstractSendChannel.SendBuffered<? extends E> node) {
        for (LockFreeLinkedListNode prevNode = node.getPrevNode(); prevNode instanceof AbstractSendChannel.SendBuffered; prevNode = prevNode.getPrevNode()) {
            if (!prevNode.remove()) {
                prevNode.helpRemove();
            }
        }
    ...
    protected Object offerInternal(E element) {
        ReceiveOrClosed<?> receiveOrClosedSendConflated;
        do {
            ...
            if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
                receiveOrClosedSendConflated = sendConflated(element);
                if (receiveOrClosedSendConflated == null) {
                    return AbstractChannelKt.OFFER_SUCCESS;
                }
            } else {
        ...
        } while (!(receiveOrClosedSendConflated instanceof Closed));
        return receiveOrClosedSendConflated;
    }
