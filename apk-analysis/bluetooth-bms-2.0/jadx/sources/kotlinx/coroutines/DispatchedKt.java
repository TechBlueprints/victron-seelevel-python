...
public final class DispatchedKt {
    ...
    public static final <T> void resumeCancellable(Continuation<? super T> resumeCancellable, T t) {
        ...
        if (dispatchedContinuation.dispatcher.isDispatchNeeded(dispatchedContinuation.getContext())) {
            ...
            dispatchedContinuation.dispatcher.mo1037dispatch(dispatchedContinuation.getContext(), dispatchedContinuation);
            return;
        }
        EventLoop eventLoop$kotlinx_coroutines_core = ThreadLocalEventLoop.INSTANCE.getEventLoop$kotlinx_coroutines_core();
        if (eventLoop$kotlinx_coroutines_core.isUnconfinedLoopActive()) {
        ...
        try {
            ...
            if (job == null || job.isActive()) {
                ...
                dispatchedContinuation.resumeWith(Result.m13constructorimpl(ResultKt.createFailure(cancellationException)));
                z = true;
            }
            if (!z) {
    ...
    public static final <T> void resumeCancellableWithException(Continuation<? super T> resumeCancellableWithException, Throwable exception) {
        ...
        if (resumeCancellableWithException instanceof DispatchedContinuation) {
            ...
            if (dispatchedContinuation.dispatcher.isDispatchNeeded(context)) {
                ...
                dispatchedContinuation.resumeMode = 1;
                dispatchedContinuation.dispatcher.mo1037dispatch(context, dispatchedContinuation);
                return;
            }
            EventLoop eventLoop$kotlinx_coroutines_core = ThreadLocalEventLoop.INSTANCE.getEventLoop$kotlinx_coroutines_core();
            if (eventLoop$kotlinx_coroutines_core.isUnconfinedLoopActive()) {
            ...
            try {
                ...
                if (!z) {
                    ...
                    try {
                        Continuation<T> continuation = dispatchedContinuation.continuation;
                        Result.Companion companion2 = Result.INSTANCE;
                        continuation.resumeWith(Result.m13constructorimpl(ResultKt.createFailure(StackTraceRecoveryKt.recoverStackTrace(exception, continuation))));
                        Unit unit = Unit.INSTANCE;
        ...
        Result.Companion companion3 = Result.INSTANCE;
        resumeCancellableWithException.resumeWith(Result.m13constructorimpl(ResultKt.createFailure(StackTraceRecoveryKt.recoverStackTrace(exception, resumeCancellableWithException))));
    }
    ...
    public static final <T> void resumeDirectWithException(Continuation<? super T> resumeDirectWithException, Throwable exception) {
        ...
        if (resumeDirectWithException instanceof DispatchedContinuation) {
            ...
            Result.Companion companion2 = Result.INSTANCE;
            resumeDirectWithException.resumeWith(Result.m13constructorimpl(ResultKt.createFailure(StackTraceRecoveryKt.recoverStackTrace(exception, resumeDirectWithException))));
        }
    }
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatch, int i) {
        ...
        if (ResumeModeKt.isDispatchedMode(i) && (delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) && ResumeModeKt.isCancellableMode(i) == ResumeModeKt.isCancellableMode(dispatch.resumeMode)) {
            ...
            if (coroutineDispatcher.isDispatchNeeded(context)) {
                coroutineDispatcher.mo1037dispatch(context, dispatch);
                return;
            } else {
    ...
    public static final void resumeWithStackTrace(Continuation<?> resumeWithStackTrace, Throwable exception) {
        ...
        Intrinsics.checkParameterIsNotNull(exception, "exception");
        Result.Companion companion = Result.INSTANCE;
        resumeWithStackTrace.resumeWith(Result.m13constructorimpl(ResultKt.createFailure(StackTraceRecoveryKt.recoverStackTrace(exception, resumeWithStackTrace))));
    }
