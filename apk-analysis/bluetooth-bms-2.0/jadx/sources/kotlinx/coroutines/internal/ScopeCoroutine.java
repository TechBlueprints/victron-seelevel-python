...
public class ScopeCoroutine<T> extends AbstractCoroutine<T> implements CoroutineStackFrame {
    ...
    protected void afterCompletionInternal(Object state, int mode) {
        if (state instanceof CompletedExceptionally) {
            Throwable thRecoverStackTrace = ((CompletedExceptionally) state).cause;
            if (mode != 4) {
                thRecoverStackTrace = StackTraceRecoveryKt.recoverStackTrace(thRecoverStackTrace, this.uCont);
            }
            ResumeModeKt.resumeUninterceptedWithExceptionMode(this.uCont, thRecoverStackTrace, mode);
            return;
        }
