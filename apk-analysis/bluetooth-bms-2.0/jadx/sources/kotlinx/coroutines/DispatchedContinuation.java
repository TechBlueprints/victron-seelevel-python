...
public final class DispatchedContinuation<T> extends DispatchedTask<T> implements CoroutineStackFrame, Continuation<T> {
    ...
    public void resumeWith(Object result) {
        ...
        if (this.dispatcher.isDispatchNeeded(context)) {
            this._state = state;
            this.resumeMode = 0;
            this.dispatcher.mo1037dispatch(context, this);
            return;
        }
    ...
    public final void resumeCancellable(T value) {
        ...
        if (this.dispatcher.isDispatchNeeded(getContext())) {
            this._state = value;
            this.resumeMode = 1;
            this.dispatcher.mo1037dispatch(getContext(), this);
            return;
        }
        EventLoop eventLoop$kotlinx_coroutines_core = ThreadLocalEventLoop.INSTANCE.getEventLoop$kotlinx_coroutines_core();
        if (eventLoop$kotlinx_coroutines_core.isUnconfinedLoopActive()) {
        ...
        try {
            ...
            if (job == null || job.isActive()) {
                ...
                resumeWith(Result.m13constructorimpl(ResultKt.createFailure(cancellationException)));
                z = true;
            }
            if (!z) {
