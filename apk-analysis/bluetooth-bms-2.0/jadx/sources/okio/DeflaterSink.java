...
public final class DeflaterSink implements Sink {
    ...
    private final void deflate(boolean syncFlush) throws IOException {
        ...
        while (true) {
            ...
            if (syncFlush) {
                iDeflate = this.deflater.deflate(segmentWritableSegment$jvm.data, segmentWritableSegment$jvm.limit, 8192 - segmentWritableSegment$jvm.limit, 2);
            } else {
                iDeflate = this.deflater.deflate(segmentWritableSegment$jvm.data, segmentWritableSegment$jvm.limit, 8192 - segmentWritableSegment$jvm.limit);
            }
            if (iDeflate > 0) {
                segmentWritableSegment$jvm.limit += iDeflate;
                buffer.setSize$jvm(buffer.size() + iDeflate);
